...
static int (*iceberg_scan_init_stream_func)(IcebergScan*, size_t, size_t, size_t, IcebergResponse*, const void*) = NULL;
...
static int load_iceberg_library(const char* library_path) {
    ...
    iceberg_scan_init_stream_func = (int (*)(IcebergScan*, size_t, size_t, size_t, IcebergResponse*, const void*))dlsym(lib_handle, "iceberg_scan_init_stream");
...
int main(int argc, char* argv[]) {
    ...
    size_t serialization_concurrency_limit = 2;
    result = iceberg_scan_init_stream_func(scan_response.scan, batch_size, concurrency_limit, serialization_concurrency_limit, &init_response, (const void*)(uintptr_t)&async_completed);
    ...
    async_completed = 0;  // Reset flag
    result = iceberg_scan_next_batch_func(scan_response.scan, &batch_response, (const void*)(uintptr_t)&async_completed);
